...
#	a=${a##* }
	echo ${a}
	~/findsecbugs/findsecbugs.sh  -high -sortByClass -xml -output ~/resultData2/${1}_${2}.xml ${target_dir}/${a}/classes
	cd ~
	rm -rf $target_dir
}
...
then
	...
	then
		...
		for bug_id in `cat ~/defects4j/framework/projects/${PID}/active-bugs.csv | tail -n +2 |awk -F, '{ print $1; }'`
		do
			scan_one ${PID} ${bug_id}
		done
	fi
